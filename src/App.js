...
class App extends React.Component {
  ...
  readData = async (id) => {
    Toast.showShortBottom(`readData call ${id}`);
    console.log(`readData: call`);
    const data = await BluetoothSerial.readOnce("\r\n");
    Toast.showShortBottom(`readData: ${data}`);
                ...
                {connected && (
                  <React.Fragment>
                    ...
                    <Text style={{ height: 40, borderColor: 'gray', borderWidth: 1 }}>
                      {JSON.stringify(this.state.inputlevel) }
                    </Text>
                    <Button
                      title="Write packets"
                      style={{
